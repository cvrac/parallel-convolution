...
int main(int argc, char **argv) {
  ...
  // Find process row and column offsets
	...
	if (grey) {
		columns_number_based_on_type = columns + 2;
		columns_contiguous = columns;
		rows_number_based_on_type = columns;
		blocklength = 1;
	} else {
		columns_number_based_on_type = 3 * (columns + 2);
		columns_contiguous = 3 * columns;
		rows_number_based_on_type = 3 * columns;
		blocklength = 3;
	}
